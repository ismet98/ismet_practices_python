...
class BankAccount:
    ...
    def deposit(self, amount: int) -> int:
        ...
        transaction = Transaction(transaction_id, before, after)
    ...
    def withdraw(self, amount: int) -> int | str:
        ...
        if amount > self.balance:
            after = self. balance
            transaction_id =len(self.transactions)
            transaction = Transaction(id,before,after)
            elf.transactions.append(transaction)
...
class Coin:
    ...
    def worth(self) -> int:
       return self.cents + max(0, Mint.present_year - self.year - 50) ## adding to the 50 of each 

class Nickel(Coin): ## this is a subclass of coint
    cents = 5
