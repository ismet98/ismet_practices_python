...
def boar_brawl(player_score, opponent_score):
    ...
    string_player_score = str(player_score).zfill(2)
    string_opponent_score = str(opponent_score).zfill(2) ## padding with 0 to ensure it is always working
    zero_brawl_score = 3 * abs(int(string_player_score[-1]) - int(string_opponent_score[-2]))

    if zero_brawl_score > 0:
        return zero_brawl_score
    elif zero_brawl_score == 0:
        return 1
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    if (num_rolls == 0):
        ...
        ##opponent_score =  roll_dice(num_rolls,dice) ## maybe this is not possible??
...
def num_factors(n):
    ...
    count_of_factors = 0
    for i in range(1,n+1): ## to get n as factor of n
        if n % i == 0:
            count_of_factors +=1
    ...
    while score0 < GOAL and score1 < GOAL:
        if who ==0:
            if update == simple_update:
               ...
               score0 = sus_update(strategy0,score0,score1,dice)
            who = 1
        elif who == 1:
            if update == simple_update:
               score1 = simple_update(strategy1,score1,score0,dice)
            elif update == sus_update:
               score1 = sus_update(strategy1,score1,score0,dice)
            who = 0
...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 7
    if strategy == always_roll(strategy):
        return True
    else: 
        return False
...
def make_averaged(original_function, times_called=1000):
    ...
    ## we can implement functions over functions
    def averaged(*args):
        total = 0
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    if sus_update(score,opponent_score) >= threshold + score:
        return 0
    else:
        return num_rolls
