...
D
        ...
        """
        current_place = self.place ## For search algorithms, we should use temporary variables to prevent ourselves from creating things that are unneccesary 
        distance = 0

        while current_place is not None:

            if (current_place.bees and not current_place.is_hive and self.lower_bound<=distance<=self.upper_bound):
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if cooldown!=0:
            ...
            if bee is not None: 
                bee.reduce_health(bee_health)
                self.cooldown = self.chew_cooldown ## setting back to the original

                ## Reduce Bee's health to 0 
                ## look at bee's class to see what we can do do to remove it to 0
# END Problem 7
...
class ContainerAnt(Ant):
    ...
    def store_ant(self, ant: Ant):
        # BEGIN Problem 8a
        self.ant_contained = ant ## this is the ant that is being contained
        # END Problem 8a
    ...
    def action(self, gamestate: GameState):
        ...
        if self.ant_contained != None:
            self.ant_contained.action(gamestate) ## But does this not do a reciprocal loop
        # END Problem 8a
...
class Water(Place):
    ...
    def add_insect(self, insect: Insect):
        ...
        if insect.is_waterproof == False:
            insect.reduce_health(insect, insect.health) ## removes all the health taken
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate: GameState):
        ...
        """
        # BEGIN Problem 12
        super().action(gamestate) ## do the same addition
        ## Tunnel Beginning 

        dummy_place = self.place
        while dummy_place!= none:
            dummy_place = dummy_place.entrance
        
        for x in dummy_place:
            if type(Ant) in x: 
                if x.ant.is_doubled == False:
                    x.ant.damage *=2
    ...
    def reduce_health(self, damage_taken: float):
        ...
        """
        # BEGIN Problem 12
        super().reduce_health(self, damage_taken)
