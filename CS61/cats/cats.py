...
def pick(paragraphs: list[str], select, k: int) -> str:
    ...
    for item in paragraphs:
        if select(item) == True:
            list_of_ok.append(item)
    
    if len(list_of_ok) > k: ## must be at least K 
        return list_of_ok[k]
    else: 
        return False
    # END PROBLEM 1
...
def about(keywords: list[str]):
    ...
    # BEGIN PROBLEM 2
    def select(paragraph): ## how we return a function
...
def accuracy(entered: str, source: str) -> float:
    ...
    # BEGIN PROBLEM 3
    if entered_words is None and source_words is None:
        return 100
    elif entered_words is None and source_words is not None:
        return 0
    elif entered_words is not None and source_words is None:
        return 0
    elif len(entered_words) > len (source_words):
        count = 0
        for item in range(0,len(source)):
            if entered_words[item] == source_words[item]:
                count += 1
        return count / len(entered_words)
    else:
        count = 0
        for item in range(0,source_words):
            if entered_words[item] == source_words[item]:
                count += 1
        return count / len(entered_words)
...
def autocorrect(entered_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    """
    # BEGIN PROBLEM 5
    accuracy_of_word = 100
    index_of_word = 0

    for item in word_list:
        diff_accuracy = diff_function(entered_word, item, limit)
        if diff_accuracy < accuracy_of_word:
            accuracy_of_word =  diff_accuracy
            word_to_be_returned = item
...
def furry_fixes(entered: str, source: str, limit: int) -> int:
    ...
    #difference = (max(len(entered),len(source))) - (min(len(entered),len(source)))\
    
    #changes += difference
    
    # Stop early if already over limit
    if limit < 0:
        return 1
